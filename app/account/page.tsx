...
function generatePassword(): string {
  ...
  // อย่างน้อย 2 ตัวเลข + 6 ตัวอักษร แล้ว shuffle
  let pwd = "";
...
export default function AccountPage() {
  ...
  // preview username ขณะพิมพ์ชื่อ
  useEffect(() => {
    if (fn.trim()) {
      setPreviewUn(generateUsername(fn));
    } else {
      setPreviewUn("");
    }
  }, [fn]);

  // สร้าง password ครั้งแรก
  useEffect(() => {
    setPreviewPw(generatePassword());
  }, []);

  async function loadUsers() {
    const { data, error } = await supabase.from("users").select("*").order("fullname");
    if (error) {
      showMsg(`❌ โหลดข้อมูลไม่สำเร็จ: ${error.message}`, "error");
      return;
    }
    setUsers(data || []);
  }
  ...
  async function handleAdd() {
    if (!fn.trim()) {
      showMsg("⚠️ กรุณากรอกชื่อ-นามสกุล", "error");
      return;
    }
    ...
    if (dup) {
      showMsg(`❌ Username "${finalUn}" มีในระบบแล้ว กรุณาแก้ชื่อให้ต่างออกไป`, "error");
      return;
    }
    ...
    if (error) {
      showMsg(`❌ บันทึกไม่สำเร็จ: ${error.message}`, "error");
      setSaving(false);
      return;
    }
    ...
    setPreviewPw(generatePassword()); // สร้าง password ใหม่สำหรับคนถัดไป
    setSaving(false);
  }
  ...
  async function handleSaveEdit() {
    if (!editFn.trim() || !editUn.trim()) {
      showMsg("⚠️ ชื่อ-นามสกุล และ Username ห้ามว่าง", "error"); return;
    }
    ...
    if (error) {
      showMsg(`❌ อัปเดตไม่สำเร็จ: ${error.message}`, "error");
      setSaving(false);
      return;
    }
  ...
  async function handleDelete(target: any) {
    ...
    if (error) {
      showMsg(`❌ ลบไม่สำเร็จ: ${error.message}`, "error"); return;
    }
    await loadUsers();
  ...
  async function handleResetPassword(target: any) {
    ...
    if (error) {
      showMsg(`❌ Reset ไม่สำเร็จ: ${error.message}`, "error"); return;
    }
    await loadUsers();
    showMsg(`🔑 Reset password ของ "${target.fullname}" สำเร็จ  |  Password ใหม่: ${newPw}`, "success");
  }
